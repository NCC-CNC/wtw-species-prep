...
prop_field = arcpy.GetParameterAsText(3)
...
# A FASTER APPROACH TO SELECT BY LOCATION --------------------------------------
# Add new burn field
...
parks_1km = "{}/Existing_Conservation".format(fgdb) # DELETE LATER
x_parks = arcpy.analysis.Intersect([m, parks_1km], "{}/INTERSECTION".format(fgdb))
 
# Build dictionary: HA
arcpy.AddMessage("... Building Range_ha field")
ha = {}
with arcpy.da.SearchCursor(x_parks, ["pointid", "SHAPE@AREA"]) as cursor:
    for row in cursor:
        id, area = row[0], row[1]
        if id not in ha:
...
with arcpy.da.UpdateCursor(parks_1km, ["pointid", "Range_ha"]) as cursor:
    for row in cursor:
        id = row[0]
        if id in ha:
            row[1] = ha[id]
